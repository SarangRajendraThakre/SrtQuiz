...
const HistorySlider = () => {
    ...
    useEffect(() => {
        const fetchHistoryQuizzes = async () => {
            setLoading(true);
            try {
                // FIX 1: Fetch ONLY public History quizzes in one efficient call.
                const response = await axios.get(`${baseUrl1}/api/quizzes/public?category=History`);
                setQuizzes(response.data.quizzes || []);
            } catch (error) {
                console.error('Error fetching History quizzes:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) {
        return (
            <div className="category-slider-container">
                <h2 className="category-slider-title">History Quizzes</h2>
