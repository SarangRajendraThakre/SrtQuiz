...
const GkSlider = () => {
    ...
    useEffect(() => {
        const fetchGkQuizzes = async () => {
            setLoading(true);
            try {
                // FIX 1: Fetch ONLY public GK quizzes in one efficient call.
                const response = await axios.get(`${baseUrl1}/api/quizzes/public?category=GK`);
                setQuizzes(response.data.quizzes || []);
            } catch (error) {
                console.error('Error fetching GK quizzes:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) {
        return (
            <div className="category-slider-container">
                <h2 className="category-slider-title">GK Quizzes</h2>
