...
const MathsSlider = () => {
    ...
    useEffect(() => {
        const fetchMathsQuizzes = async () => {
            setLoading(true);
            try {
                // FIX 1: Fetch ONLY public Maths quizzes in one efficient call.
                const response = await axios.get(`${baseUrl1}/api/quizzes/public?category=Maths`);
                setQuizzes(response.data.quizzes || []);
            } catch (error) {
                console.error('Error fetching Maths quizzes:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) {
        return (
            <div className="category-slider-container">
                <h2 className="category-slider-title">Maths Quizzes</h2>
