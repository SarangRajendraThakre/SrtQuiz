...
const BiologySlider = () => {
    ...
    useEffect(() => {
        const fetchBiologyQuizzes = async () => {
            setLoading(true);
            try {
                // FIX: Fetches ONLY public Biology quizzes in one efficient call.
                const response = await axios.get(`${baseUrl1}/api/quizzes/public?category=Biology`);
    ...
    // FIX: Uses the single, efficient backend endpoint for copying.
    ...
    if (loading) {
        return (
            <div className="category-slider-container">
                <h2 className="category-slider-title">Biology Quizzes</h2>
