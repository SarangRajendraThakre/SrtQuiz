...
const PhysicsSlider = () => {
    ...
    useEffect(() => {
        const fetchPhysicsQuizzes = async () => {
            setLoading(true);
            try {
                // FIX 1: Fetch ONLY public Physics quizzes in one efficient call.
                const response = await axios.get(`${baseUrl1}/api/quizzes/public?category=Physics`);
                setQuizzes(response.data.quizzes || []);
            } catch (error) {
                console.error('Error fetching Physics quizzes:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) {
        return (
            <div className="category-slider-container">
                <h2 className="category-slider-title">Physics Quizzes</h2>
                    ...
                    <div key={quiz._id} className='quiz-card-wrapper'>
                        <div className='quiz-card'>
                            <div className='quiz-card-image-container'>
                                <img src={`${baseUrl1}${quiz.posterImg}`} alt={quiz.title} className='quiz-card-image' />
