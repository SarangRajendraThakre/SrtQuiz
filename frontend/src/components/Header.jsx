    ...
    <>
      <div className="w-full fixed h-14 flex z-40 bg-white justify-between items-center font-sans px-4">
          ...
          <>
            <button onClick={handleToggleModalSetting}>
              <div className="flex h-10 justify-between bg-white rounded-md border border-gray-400 justify-center items-center w-96 text-gray-700 font-bold">
                <span className="pl-4">
                  {createdquizdatatitle ? createdquizdatatitle : "Enter kahoot title..."}
                </span>
                <div className="bg-gray-200 px-4 py-2 ml-2 text-sm mr-4">Setting</div>
              </div>
            </button>
        ...
        {!isLoginOrRegisterPage() && (
          <>
            ...
            <button className="bg-blue-600 flex items-center text-white font-semibold rounded-md px-2 mx-1">
              <IoIosColorPalette />
              <span className="flex items-center justify-center w-24">Themes</span>
            </button>
        ...
        {user ? (
          ...
          <Link to='/register' className="bg-gray-200 h-9 px-4 pb-1 font-semibold mr-2 rounded-md">Register</Link>
        )}
        {user ? (
          ...
          <Link to='/login' className="bg-gray-200 h-9 px-4 pb-1 font-semibold mr-2 rounded-md">Login</Link>
        )}
      </div>
    </>
  );
